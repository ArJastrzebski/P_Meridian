...
def result_factors_chart(result_factors):
    ...
    fig_rs = px.bar(result_factors_df, x='Factor', y='Percentage', 
                      title='Sales contribution by baseline and marketing channel', color_discrete_sequence=['#0050B5'], width=600, height=350)   
...
def marketing_funnel_chart(demo_data):
    ...
    fig_mf = px.funnel(funnel_data, x='number_log', y='stage', text=[f'{stage}: {value}' for stage, value in zip(funnel_data['stage'], funnel_data['number'])],
                         title='Marketing funnel and daily conversion rates', color_discrete_sequence=['#3C5291'], labels=None, width=600, height=350)
...
def base_chart(demo_data):
    ...
    fig_base = px.area(data_grouped, x=group_by, y=(data_grouped.columns), color_discrete_sequence=px.colors.sequential.Sunset, width=600, height=350) # sorted(data_source.columns[:-1]
