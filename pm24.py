...
def expected_leads_chart(demo_data, group_by):
    ...
    fig_exl = px.line(data_grouped, x=group_by, y=['Expected_leads', 'Actual_leads'], 
        labels={'value': 'Values', 'time': group_by}, title='Expected leads vs. Actual leads',
                      color_discrete_map= {'Expected_leads': '#135DD8', 'Actual_leads': '#D6001C'}, width=600, height=300)
...
def result_factors_chart(result_factors):
    ...
    fig_rs = px.bar(result_factors_df, x='Factor', y='Percentage', 
                      title='Sales contribution by baseline and marketing channel', color_discrete_sequence=['#0050B5'], width=600, height=300)   
...
def marketing_funnel_chart(demo_data):
    ...
    fig_mf = px.funnel(funnel_data, x='number_log', y='stage', text=[f'{stage}: {value}' for stage, value in zip(funnel_data['stage'], funnel_data['number'])],
                         title='Marketing funnel and daily conversion rates', color_discrete_sequence=['#3C5291'], labels=None, width=600, height=300)
...
def base_chart(demo_data):
    ...
    fig_base = px.area(data_grouped, x=group_by, y=(data_grouped.columns), color_discrete_sequence=px.colors.sequential.Sunset, width=600, height=300) # sorted(data_source.columns[:-1]
...
def Scenario_6(demo_data): 
    ...
    with funnel_col:
        ...
        base_chart(demo_data)
with tab6: 
    Scenario_6(demo_data5)

def Scenario_7(demo_data): 
    st.markdown("""<h4 style="font-size: 20px; margin-bottom: -20px;"> Leads </h4>""", unsafe_allow_html=True)
    stat_col, summarizer_col, funnel_col = st.columns([1,2,2])
